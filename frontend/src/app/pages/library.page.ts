      ...
      @if (state() === 'loading') {
        ...
        <section class="library-list" aria-label="Arquivos disponíveis">
          @for (item of items(); track item.id) {
            <article class="library-row"><a [routerLink]="['/catalog', item.movieTmdbId]" class="library-poster">@if (item.posterPath) { <img [src]="posterUrl(item)" [alt]="'Pôster de ' + item.movieTitle" /> } @else { <i class="ph ph-film-strip"></i> }</a><div><a [routerLink]="['/catalog', item.movieTmdbId]">{{ item.movieTitle }}</a><p>{{ item.relativePath }}</p><span>{{ quality(item) }}</span></div><strong>{{ size(item.sizeBytes) }}</strong></article>
          }
        </section>
      } @else {
